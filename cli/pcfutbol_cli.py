#!/usr/bin/env python3
"""
PC Ftbol 5  Clon CLI (Python)
Temporada 2025/26    Datos reales extrados del juego original

Uso:
    python pcfutbol_cli.py

Requiere:
    Python 3.9+  (stdlib slo, sin dependencias externas)
"""
...
# ---------------------------------------------------------------------------
# Configuracin de competiciones (cdigo CSV  metadatos)
# Solo se cargan competiciones con 14 equipos en el CSV
# ---------------------------------------------------------------------------
COMP_INFO: dict[str, dict] = {
    "ES1":  {"name": "Primera Divisin",       "country": "Espaa",      "n_rel": 3, "tot_md": 38, "min_teams": 14},
    "ES2":  {"name": "Segunda Divisin",        "country": "Espaa",      "n_rel": 4, "tot_md": 42, "min_teams": 14},
    "E3G1": {"name": "1 RFEF Grupo 1",        "country": "Espaa",      "n_rel": 4, "tot_md": 38, "min_teams": 14},
    "E3G2": {"name": "1 RFEF Grupo 2",        "country": "Espaa",      "n_rel": 4, "tot_md": 38, "min_teams": 14},
    ...
    "NL1":  {"name": "Eredivisie",              "country": "Pases Bajos","n_rel": 2, "tot_md": 34, "min_teams": 14},
    "PO1":  {"name": "Primeira Liga",           "country": "Portugal",    "n_rel": 2, "tot_md": 34, "min_teams": 14},
    "BE1":  {"name": "Pro League",              "country": "Blgica",     "n_rel": 2, "tot_md": 30, "min_teams": 14},
    "TR1":  {"name": "Sper Lig",              "country": "Turqua",     "n_rel": 3, "tot_md": 34, "min_teams": 14},
}
...
class Team:
    ...
    def strength(self) -> float:
        """Fuerza global con la misma frmula base que el motor Android."""
...
class KotlinXorWowRandom:
    """
    Implementacin Python de kotlin.random.Random(seed) (XorWowRandom),
...
def _tactic_adj(tactic: Optional[dict], is_home: bool) -> float:
    """Misma frmula que StrengthCalculator.tacticalAdjustment()."""
    t = tactic or {}
...
def simulate_match(home: Team, away: Team, seed: int,
    ...
    - StrengthCalculator (misma frmula)
    - MatchSimulator (Poisson + VAR + rojas + prdida de tiempo)
    """
...
def generate_fixtures(teams: list[Team]) -> list[tuple[Team, Team]]:
    """Genera calendario con el mismo algoritmo de rotacin del engine Android."""
    if len(teams) < 2:
        return []
    rotation = list(teams)
    if len(rotation) % 2 != 0:
    ...
    while True:
        try:
            ...
            if min_val <= val <= max_val:
                return val
            print(f"  Introduce un nmero entre {min_val} y {max_val}.")
        except ValueError:
            print("  Nmero invlido.")
...
def menu_simulate_season(liga1: list[Team], liga2: list[Team]):
    ...
    print(_c(YELLOW, "\n  Simulando LIGA1 (Primera Divisin)..."))
    liga1_standings, liga1_results = simulate_season(liga1, "LIGA1", seed)

    print(_c(YELLOW, "  Simulando LIGA2 (Segunda Divisin)..."))
    ...
    print_standings(liga1_standings, "PRIMERA DIVISIN 2025/26", relegated_from=3)
    print_standings(liga2_standings, "SEGUNDA DIVISIN 2025/26", promoted_to=3)

    champion = liga1_standings[0]
    print(_c(BOLD + GREEN, f"   CAMPEN: {champion.team.name} con {champion.points} puntos"))
    ...
    while True:
        print(_c(CYAN, "\n   Ver resultados "))
        print(_c(CYAN, "   1. Jornada especfica LIGA1      "))
        print(_c(CYAN, "   2. Jornada especfica LIGA2      "))
        print(_c(CYAN, "   3. Resultados de un equipo       "))
        print(_c(CYAN, "   4. Plantilla de un equipo        "))
        print(_c(CYAN, "   0. Volver                        "))
        print(_c(CYAN, "  "))

        op = input_int("  Opcin: ", 0, 4)
        if op == 0:
            break
        elif op == 1:
...
def menu_top_players(liga1: list[Team], liga2: list[Team]):
    ...
    print(_c(CYAN, "\n  Ordenar por:"))
    print("    1. Valoracin media (ME)")
    print("    2. Delanteros (remate)")
    print("    3. Porteros")
    print("    4. Defensas")
    print("    5. Centrocampistas (pase)")

    op = input_int("  Opcin (1-5): ", 1, 5)
    n  = input_int("  Cuntos jugadores? (5-50): ", 5, 50)
...
def _generate_offers(prestige: int, liga1: list[Team], liga2: list[Team], liga_rfef: list[Team] = [], liga_foreign: dict[str, list[Team]] = {}, data: Optional[dict] = None) -> list[Team]:
    ...
    if prestige == 1:
        ...
        # Primera Divisin espaola + ligas top extranjeras (ms dbiles)
        pool = list(l1) + l_foreign[:10]
    elif prestige <= 9:
        # Mezcla de Primera espaola y todas las ligas extranjeras
    ...
    if mgr_team is not None and _play_mode_allows_president(mode):
        ...
        print(
            _c(
                ...
                f"Tope {cap_k:,}K  Masa {bill_k:,}K  Margen {_c(margin_c, f'{margin_k:+,}K')}",
            )
        )
        print(
            _c(
...
def _news_item(r: dict, tbs: dict[int, Team], mgr_slot: int,
    ...
    if mg > og:
        ...
        extra = " La junta convoca urgencia." if drop else f" Posicin {pos}."
...
def _season_end_screen(data: dict, standings: list[Standing], mgr_slot: int,
    ...
    print(_c(BOLD + YELLOW, "  "))
    print()
    print(_c(GREEN, f"   CAMPEN: {standings[0].team.name} ({standings[0].points} pts)"))
    print()
    print(_c(YELLOW, f"  TU RESULTADO  {mgr_team.name}"))
    if mgr_st:
        print(f"  Posicin: {mgr_pos} / {total}")
    ...
    if mgr_team in rel_teams:
        ...
        if comp_key == "ES2":
            print(_c(GREEN, f"\n    {mgr_team.name} ASCIENDE a Primera Divisin"))
        elif comp_key in ("E3G1", "E3G2"):
            print(_c(GREEN, f"\n    {mgr_team.name} ASCIENDE a Segunda Divisin"))
        else:
...
def _tactic_summary(t: dict) -> str:
    ...
    pt = "  [PRD.TIEMPO]" if t.get("perdidaTiempo", 0) == 1 else ""
    return f"{tj}  {tm}  Presin {tp}  Toque {t.get('porcToque',50)}%  Contra {t.get('porcContra',30)}%{pt}"
...
def _tactic_menu(data: dict):
    ...
    while True:
        print(_c(GRAY, f"\n  Ajuste tctico actual: {_tactic_adj(t, is_home=False):+.1f}"))
        ...
        print(f"  3. Presin          : {_c(CYAN, _TACTIC_LABELS['tipoPresion'][t['tipoPresion']])}")
        ...
        print(f"  8. Prdida de tiempo: {pt_lbl}  {_c(GRAY, '(ms amarillas  +tiempo aadido)')}")
        print(_c(CYAN, "  0. Volver"))

        op = input_int("  Opcin: ", 0, 8)
        if op == 0:
            _save_career(data)
            break
        elif op == 1:
...
def _entrenador_substitution(mgr_team: "Team", subs_made: list) -> bool:
    ...
    print(_c(GRAY, f"  {'#':>3}  {'JUGADOR':<26} {'POSICIN':<18} {'ME':>3}"))
    print(_c(GRAY, "  " + "" * 52))
...
def _market_buy(data: dict, mgr_team: Team, all_slots: dict[int, Team],
    ...
    print(_c(CYAN, "\n  Filtrar por posicin: 1.Todos  2.Porteros  3.Defensas  4.Medios  5.Delanteros"))
    ...
    if not cap_ok:
        print(
            _c(
                ...
                f"masa proyectada {projected_k:,}K/sem > tope {cap_k:,}K/sem.\n",
            )
        )
        return
...
def _market_menu(data: dict, mgr_team: Team, all_slots: dict[int, Team],
    ...
    print(
        _c(
            GRAY,
            f"  Tope salarial: {cap_k:,}K/sem  |  Masa actual: {bill_k:,}K/sem  "
...
def _winter_market_menu(data: dict, mgr_team: Team, comp_teams: list[Team], md: int):
    ...
    ans = input_str("  Quieres fichar algn jugador? (S/N): ").strip().upper()
    if not ans.startswith("S"):
        print(_c(GRAY, "  Contina la temporada sin movimientos.\n"))
        return
    ...
    if idx == 0:
        print(_c(GRAY, "  Operacin cancelada.\n"))
        return
...
def _season_loop(data: dict, liga1: list[Team], liga2: list[Team], liga_rfef: list[Team] = [],
    ...
    while cur_md <= tot_md:
        ...
        if my_fix:
            h, a = my_fix
            print(_c(CYAN, f"  PRXIMO PARTIDO  Jornada {cur_md}:"))
        ...
        print(_c(GRAY, f"  Tctica: {_tactic_summary(tactic)}"))
        ...
        print(_c(CYAN,  "  2. Clasificacin completa"))
            ...
            for h, a in fix_by_md.get(cur_md, []):
                ...
                if is_mgr_match:
                    if _play_mode_allows_coach_match(play_mode):
                        print(_c(CYAN, "\n  Modo de partido:"))
                        print(_c(CYAN, "  1. Simular automticamente"))
                        print(_c(CYAN, "  2. Modo Entrenador  (fichas animadas)"))
                        pm = input_int("  Opcin (1-2): ", 1, 2)
                        if pm == 2:
...
def _comp_name(comp: str) -> str:
    ...
    if info:
        ...
        name = info["name"]
        return name if country == "Espaa" else f"{name} ({country})"
    return comp
...
def main_menu():
    print(_c(BOLD + CYAN,  "\n  "))
    print(_c(BOLD + CYAN,  "      PC FTBOL 5    CLI  2025/26  "))
    ...
    print(_c(GREEN, f"   {len(liga1)} equipos en Primera Divisin"))
    print(_c(GREEN, f"   {len(liga2)} equipos en Segunda Divisin"))
    print(_c(GREEN, f"   {len(liga_rfef)} equipos en 1 RFEF"))
